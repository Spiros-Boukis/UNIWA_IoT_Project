...
const char* mqtt_server = "spirosboukis.ddns.net";
//ID του MQTT Client
const char* mqttClientID = "room2_ESP";
//LWT
const char* willTopic = "room2/sensor/status";
...
void loop() {
 ...
 //μεταβλητές
  ...
  if (now - lastMsgTime > measurement_interval) {
    ...
    //
    client.publish("room2/temperature", tempStr,true);
    client.publish("room2/humidity", humStr,true);
...
void setup_mqtt(){
  ...
  client.setCallback(mqtt_message_callback);
   //--1.6--
  client.subscribe("room2/led/output",1);
  //--1.7
  client.publish("room2/led/status", "false", true);
  client.publish("room2/sensor/status", "online", true);
  }
}
...
void mqtt_message_callback(char* topic,byte* message,unsigned int length){
  ...
  //Έλεγχος topic
  if (String(topic) == "room2/led/output") {
    ...
    if (arrivedMsg == "true") {
      ...
      client.publish("room2/led/status", ledStatus, true);
    ...
    Serial.print("Published to room2/led/status payload:");
    Serial.println(arrivedMsg);
  }
}
